...
├── components/
│   ├── Login/
│   │   ├── LoginContainer.tsx    # Shell that handles auth states & connects hooks
│   │   └── LoginForm.tsx         # Presents login controls via useAppForm
│   └── Registration/
│       ├── RegistrationContainer.tsx  # Maps form submissions to registration hooks
...
### Autogenerated API Clients (`src/api/generated/`)

Instead of writing fetch wrappers manually under `features/Auth/api/`, all client SDK code, models, and queries are automatically generated directly from our backend's OpenAPI schema using **Orval**:

- **Generated Client**: `src/api/generated/openapi.ts` contains fully typed hooks (e.g., `usePostApiV1AuthLogin`, `usePostApiV1Users`) and request/response type definitions.
- **Architecture**: The **container** components import these autogenerated hooks directly to invoke mutations and queries, keeping the feature files clean and presentational.
...
TanStack Query (`@tanstack/react-query`) is installed, configured, and fully populated. The client infrastructure includes:
...
### Autogenerated Query Hooks

All React Query hooks for querying and mutating data are autogenerated by **Orval** inside `src/api/generated/openapi.ts`. There is no need to manually compose `useQuery` or `useMutation` wrappers.

Example usage in a container:

```typescript
import { usePostApiV1AuthLogin } from "@/api/generated/openapi";
